...
class ElasticClient:
    ...
    def __init__(self, url, ca, tls_enabled, dry_run=False):
        ...
        self.es = elasticsearch.Elasticsearch(
            [url], verify_certs=verify_certs, ca_certs=ca
        )
    ...
    def send(self, index, doc_type, data):
        ...
        """
        if self.dry_run:
            logger.info("Dry-run: index=%s doc_type=%s data=%s", index, doc_type, data)
            return
...
class ElasticsearchConsumer(object):
    ...
    def __init__(
        self, url, index, dry_run, elastic_user, elastic_password, tls_enabled
    ):
        self._parse_config(
            url, index, dry_run, elastic_user, elastic_password, tls_enabled
        )
        self._connect()
    ...
    def _parse_config(
        ...
        self.index = "{}-{}".format(self.index, date.today().strftime("%Y.%m.%d"))
    ...
    def _connect(self):
        ...
        self.es_location = "{}://{}:{}@{}".format(
            protocol, self._elatic_user, self._elatic_password, self.url
        )
        self.es_client = ElasticClient(
            url=self.es_location,
            ca=certifi.where(),
    ...
    def create_index(self, data):
        ...
        self.es_client.es.indices.create(
            index=self.index, body=data, ignore=400  # ignore 400 already exists code
        )

    def send_to_es(self, query_name, data):
        logger.info("Sending data to ES: {}".format(query_name))
        self.es_client.send(index=self.index, doc_type=self.DOC_TYPE, data=data)
